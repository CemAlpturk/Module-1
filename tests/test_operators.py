...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert 0.0 <= minitorch.operators.sigmoid(a) <= 1.0
    assert_close(minitorch.operators.sigmoid(-a), 1.0 - minitorch.operators.sigmoid(a))
    assert_close(minitorch.operators.sigmoid(0.0), 0.5)

    # Curve flattens as x grows so numerically it is not strictly increasing
    # assert minitorch.operators.sigmoid(a) < minitorch.operators.sigmoid(a + 1.0)
